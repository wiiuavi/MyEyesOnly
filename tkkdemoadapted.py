...
def setup_demo(master):
    ...
    style = ttk.Style()
    # use 'darkly' by default so the demo starts in the requested dark theme
    style.theme_use('darkly')
    theme_names = style.theme_names()

    # show current theme (fixed)
    theme_selected = ttk.Label(
        master=root, text=style.theme.name, font="-size 24 -weight bold"
    )
    theme_selected.pack(anchor=W, pady=(0, 6))

    ttk.Separator(root).pack(fill=X, pady=10, padx=10)

    # theme selection removed; demo runs in the configured default theme
    ...
    # City table removed — kept the frame (`ttframe`) for layout only.
    # Former code created a Treeview of sample cities; it was removed
    # to simplify the demo per user request.
    # If you want to restore it, re-add a Treeview here and populate it.
